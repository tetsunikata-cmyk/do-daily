class RoadmapsController < ApplicationController
 ...
 def show
  ...
  # "2030年 世界大会優勝" のようなテキストを
  # { year: 2030, title: "世界大会優勝" } の配列に変換
  ...
  # ==== 年間スケジュール（1〜12月 × 予定・目標・結果） =========

  # "1月,地方大会①,ベスト４,ベスト８" のようなテキストを
  # { 1 => { plan: "...", target: "...", result: "..." }, ... } に変換
  def parse_yearly_schedule(text)
    months = {}
    (1..12).each do |m|
      months[m] = { plan: "", target: "", result: "" }
    end

    text.to_s.split("\n").each do |line|
      parts = line.split(",", 4).map(&:strip)
  ...
  def build_yearly_schedule(schedule_param)
    (1..12).map do |month|
      ...
      result = (row[:result] || row["result"]).to_s.strip

      # 何も入ってない月は保存しない
      next if plan.blank? && target.blank? && result.blank?

      "#{month}月,#{plan},#{target},#{result}"
    end.compact.join("\n")
  end
end