class ScheduleEventsController < ApplicationController
  ...
  def create
    ...
    if event.save
      redirect_to habit_schedule_day_path(@habit, date: event.starts_at.to_date),
                  notice: "予定を追加しました"
    else
      redirect_to habit_schedule_day_path(@habit, date: Date.current),
  ...
  def destroy
    ...
    @event.destroy
    redirect_to habit_schedule_day_path(@habit, date: date),
                notice: "予定を削除しました"
  end
